...
def calculate_displacement(current_pos, initial_pos, time, period):
    # Calculate expected position
    angle = 2 * np.pi * time / period
    expected_pos = np.array([
        initial_pos[0] * np.cos(angle) - initial_pos[1] * np.sin(angle),
        initial_pos[0] * np.sin(angle) + initial_pos[1] * np.cos(angle)
    ])
    # Calculate displacement
    displacement = np.linalg.norm(current_pos - expected_pos)
    return displacement
...
def simulate_gpu(num_planets, years, base_dt, perturbation_angle):
    ...
    initial_positions = positions[1:].copy()
    ...
    while total_time < target_time:
        ...
        if step % 100 == 0:
            ...
            for i in range(1, num_bodies):
                displacement = calculate_displacement(positions[i], initial_positions[i-1], total_time, periods[i-1])
            ...
            #logging.info(f"Time: {total_time/YEAR:.2f} years, Max Displacement: {max_displacement/AU:.2e} AU, "
            #             f"Energy Dev: {energy_dev:.2e}, Angular Momentum Dev: {am_dev:.2e}")

    return max_displacement/AU, energy_deviation, angular_momentum_deviation
...
def run_simulations(num_bodies, num_years):
    ...
    for N in planet_numbers:
        ...
        start = time.time()
        max_displacement, energy_dev, am_dev = simulate_gpu(N, years=num_years, base_dt=1*24*3600, perturbation_angle=0.1)
        #print(f"max_displacement={max_displacement} AU for {N} planets in {time.time() - start} seconds.")
        stability_measures.append(max_displacement)
...
def plot_results(planet_numbers, stability_measures, energy_deviations, angular_momentum_deviations):
    ...
    ax1.set_ylabel('Maximum Displacement (AU)')
